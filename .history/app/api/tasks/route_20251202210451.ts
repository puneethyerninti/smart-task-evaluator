...
export async function POST(request: Request) {
  ...
  requestEvaluation({
    ...
    console.error('Edge evaluation failed, triggering server fallback', err);
    setTimeout(async () => {
      try {
        await fetch(`${process.env.NEXT_PUBLIC_APP_URL}/api/run-evaluation`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
          body: JSON.stringify({ task_id: task.id }),
        });
      } catch (fallbackErr) {
        console.error('Server evaluation fallback also failed', fallbackErr);
      }
    }, 1000);
  });
