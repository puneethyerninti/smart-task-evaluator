...
export async function POST(request: Request) {
  ...
  if (insertError || !createdTask) {
    ...
    const normalizeError = (e: unknown) => {
      if (!e || typeof e !== 'object') return { message: String(e ?? 'Failed to create task'), details: null, hint: null, code: null };
      const obj = e as Record<string, unknown>;
      const message = typeof obj.message === 'string' ? obj.message : typeof obj.error === 'string' ? obj.error : 'Failed to create task';
    ...
    const devDetails = normalizeError(insertError);
  ...
  // Trigger evaluation in background - do not await
  const evalUrl = `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/run-evaluation`;
  console.log('Triggering evaluation at:', evalUrl, 'for task:', task.id);
  fetch(evalUrl, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
    body: JSON.stringify({ task_id: task.id }),
  })
    .then((res) => console.log('Eval triggered, status:', res.status))
