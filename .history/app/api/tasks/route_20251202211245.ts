...
export async function POST(request: Request) {
  ...
  // Trigger evaluation asynchronously but don't wait
  (async () => {
    try {
      console.log('Triggering evaluation for task:', task.id);
      const evalResponse = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/run-evaluation`, {
        method: 'POST',
