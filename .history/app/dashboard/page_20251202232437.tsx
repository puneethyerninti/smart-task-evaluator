...
type ReportSummary = { id: string; task_id: string | null; score: number | null; unlocked: boolean; created_at: string | null };
    ...
    return (
      <main className="min-h-screen flex items-center justify-center p-10 text-center bg-slate-950 text-white">
        <div className="max-w-lg space-y-6">
          <h1 className="text-4xl font-bold">Configuration Required</h1>
          <p className="text-slate-300 text-sm">Set <code className="font-mono">NEXT_PUBLIC_SUPABASE_URL</code> and <code className="font-mono">NEXT_PUBLIC_SUPABASE_ANON_KEY</code> in Vercel Project Settings → Environment Variables, then redeploy.</p>
          <Link href="/billing" className="inline-flex items-center justify-center rounded-full bg-indigo-600 px-5 py-2 text-sm font-semibold shadow hover:bg-indigo-500">View instructions</Link>
        </div>
      </main>
    );
  }
