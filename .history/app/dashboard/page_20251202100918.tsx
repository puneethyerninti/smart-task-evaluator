              ...
              {(tasks ?? []).slice(0, 6).map((task: TaskSummary) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`mt-3 inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold ${badgeClasses}`}
                    >
        ...
        <section className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-100">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-slate-500">
                Pay to unlock full insights and download assets.
              </p>
            </div>
                ...
                >
                  <div>
                    <p className="text-sm font-semibold text-slate-900">
                      Report #{evaluation.id.slice(0, 8)}
                    </p>
                    <p className="text-xs text-slate-500">
                      Score: {evaluation.score ?? "—"} ·{" "}
                      {evaluation.unlocked ? "Unlocked" : "Locked"}
                    </p>
                  </div>
                  <span className="text-xs text-slate-400">
                    {evaluation.created_at
                      ? dateFormatter.format(new Date(evaluation.created_at))
                      : "—"}
                  </span>
                </Link>
              ))}
